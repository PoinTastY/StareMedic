...
namespace StareMedic.Models.Documents
{
    public static class GenerateAdmisionDoc
    {
        ...
        private static void AdmisionClinica(Canvas canvas, CasoClinico caso)
        {
            ...
            var fecha_ingreso = new Paragraph($"{caso.FechaIngreso:dd/MM/yyyy}").SetMaxWidth(160).SetFixedPosition(columna_1, 770, 160);
            canvas.Add(fecha_ingreso);

            var sexo = new Paragraph($"{s_patient.Sexo}").SetMaxWidth(50).SetFixedPosition(columna_2, 770, 50);
            canvas.Add(sexo);

            var edad = new Paragraph($"{s_patient.Edad}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_3, 770, 160);
            canvas.Add(edad);

            var nombre_paciente = new Paragraph($"{s_patient.Nombre}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 755, 160);
            ...
            var nacionalidad = new Paragraph($"{s_patient.Nacionalidad}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_3, 755, 160);
            canvas.Add(nacionalidad);

            var domicilio_paciente = new Paragraph($"{s_patient.Domicilio}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 730, 160);
            canvas.Add(domicilio_paciente);

            var telefono_paciente = new Paragraph($"{s_patient.Telefono}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_2, 730, 160);
            canvas.Add(telefono_paciente);

            var ciudad_estado = new Paragraph($"{s_patient.Ciudad}, {s_patient.Estado}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 715, 160);
            ...
            var telefono_cercano = new Paragraph($"{s_cercano.Telefono}").SetMaxWidth(160).SetFixedPosition(columna_3, 715, 160);
            canvas.Add(telefono_cercano);

            var nombre_cercano = new Paragraph($"{s_cercano.Nombre}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 695, 160);
            ...
            var ciudad_estado_cercano = new Paragraph($"{s_cercano.Ciudad}, {s_cercano.Estado}").SetMaxWidth(150).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 665, 150);
            ...
            var nombre_doctor = new Paragraph($"{s_medic.Nombre}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 650, 160);
            canvas.Add(nombre_doctor);

            var nombre_fiador = new Paragraph($"{s_cercano.Nombre}").SetMaxWidth(160).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 630, 160);
            canvas.Add(nombre_fiador);

            var telefono_fiador = new Paragraph($"{s_cercano.Telefono}").SetMaxWidth(150).SetFixedPosition(columna_2, 635, 150);
            ...
            var ciudad_estado_fiador = new Paragraph($"{s_cercano.Ciudad}, {s_cercano.Estado}").SetMaxWidth(150).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 600, 140);
            canvas.Add(ciudad_estado_fiador);

            var diagnostico = new Paragraph($"{s_diag.Contenido}").SetMaxWidth(450).SetMaxHeight(altura_parrafo).SetFixedPosition(columna_1, 580, 450);
